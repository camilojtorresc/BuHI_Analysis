...
void BuFit_ptbins(Double_t ptl=7.0, Double_t pth=50.0)
{
...
void DoFit(RooDataSet data,  RooRealVar M, Double_t maxM, Double_t minM, TString Lumi, Double_t ptl, Double_t pth)
{
 ...
 //**************************
 // **** Resolution from MC ****
 Double_t Muval1,Gg1val,Gg2val,fg1val;
 
 ifstream entrada1 (Form("MCanalysis/plots_ptbinsMC/parameters_BuFit_ptbins_%1.0f_%1.0f.txt",ptl,pth));
 //entrada1.is_open();
 if ( !entrada1 ) 
    {
 ...
 entrada1>> Muval1 >> Gg1val >> Gg2val  >> fg1val  ;
 entrada1.close();
 cout << "resolution values: " << Muval1 << " " << Gg1val << " " << Gg2val << " " << fg1val << endl;
 //return;
 
 // **** gaussians ****
 RooRealVar mean("mean"," Mass mean",5.279,5.250,5.3,"GeV");
 //RooRealVar width("width"," Mass width",0.015,0.005,0.030,"GeV");
 RooRealVar width("width"," Mass width",Gg1val,0.005,0.030,"GeV");
 RooGaussian Sig("Sig"," Signal PDF",M,mean,width);

 //RooRealVar width2("width2"," Mass width2 ",0.040,0.020,0.07,"GeV");
 RooRealVar width2("width2"," Mass width2 ",Gg2val,0.020,0.07,"GeV");
 ...
 //RooRealVar fs("fs","fs",fg1val,0.,1.);
 RooRealVar fs("fs","fs",fg1val,0.,1.);
...
//------------ Fit procedure -------------------
 ...
 ofstream salida_nominal(Form("plots_ptbins/output_BuFit_ptbins_%1.0f_%1.0f.txt",ptl,pth));
 ...
 // *********************************************************
 // consider pol1 as alternative bkg model
 // *********************************************************
 cout<< "using alternative bkg model: RooChebychev pol1 function \n"<<endl;

 RooRealVar a0("a0","a0",-0.6,-10.0,10.0);
 //RooChebychev bkg2("bkg2","Background",M,RooArgList(a0)); 
 RooAbsPdf *bkg2 = 0;
 bkg2 = new RooChebychev("bkg2","Pol. Background",M,a0);
 
 RooAbsPdf *sumbkg2 =0;
 sumbkg2 = new RooAddPdf("sumbkg2","add Erf and bkg2",RooArgList(*genpdf,*bkg2),RooArgList(fb));
 
 RooAbsPdf *MassModel2 = 0;
 MassModel2 = new RooAddPdf("MassModel2","MassModel2",RooArgList(*sumgau,*sumbkg2),RooArgList(Ns,Nb));

 RooFitResult* fitres2 = MassModel2->fitTo(data,Extended(),Minos(kFALSE),Save(kTRUE), NumCPU(4));
 fitres2->Print("v");

 //parameters output file
 ofstream salida_sysbkg(Form("plots_ptbins/output_BuFit_ptbins_sysbkg_%1.0f_%1.0f.txt",ptl,pth));
 salida_sysbkg.is_open();
 save_result(salida_sysbkg, Ns, Nb);

 TCanvas* canv_sysbkg = CreateCanvasNom("canv_sysbkg", fitres2, data, M, supM, infM, MassModel2, sumgau, bkg2, genpdf, Ns, Nb, width, width2, fs, mean, ptl, pth, Lumi, 0); 
 canv_sysbkg->Print(Form("plots_ptbins/mass_BuFit_ptbins_sysbkg_%1.0f_%1.0f.png",ptl,pth));
 canv_sysbkg->Print(Form("plots_ptbins/mass_BuFit_ptbins_sysbkg_%1.0f_%1.0f.pdf",ptl,pth));


 // *********************************************************
 // considere resolution free as alternative signal model
 // *********************************************************
 cout<< "using alternative signal model: resolution parameters free \n"<<endl;
 width.setConstant(kFALSE);
 width2.setConstant(kFALSE);
 fs.setConstant(kFALSE);

 RooFitResult* fitres4 = MassModel->fitTo(data,Extended(),Minos(kFALSE),Save(kTRUE), NumCPU(4));
 fitres4->Print("v");

 //parameters output file
 ofstream salida_syssig(Form("plots_ptbins/output_BuFit_ptbins_syssig_%1.0f_%1.0f.txt",ptl,pth));
 salida_syssig.is_open();
 save_result(salida_syssig, Ns, Nb);

 TCanvas* canv_syssig = CreateCanvasNom("canv_syssig", fitres4, data, M, supM, infM, MassModel, sumgau, bkg1, genpdf, Ns, Nb, width, width2, fs, mean, ptl, pth, Lumi, 0); 
 canv_syssig->Print(Form("plots_ptbins/mass_BuFit_ptbins_syssig_%1.0f_%1.0f.png",ptl,pth));
 canv_syssig->Print(Form("plots_ptbins/mass_BuFit_ptbins_syssig_%1.0f_%1.0f.pdf",ptl,pth));
