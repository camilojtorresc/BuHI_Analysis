...
TCanvas* CreateCanvasNom(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAddPdf MassModel, RooAddPdf sumgau, RooExponential bkg1, RooGaussian genpdf, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, TString Lum, int j)
{
  ...
  // Legends para cortes del rapidity y el pt

  if(j==1)
    {
      legpar->AddEntry("",Form("|Y(B^{+})| < 0.8"),"");
    }
  if(j==2)
    {
      legpar->AddEntry("",Form("0.8 #leq |Y(B^{+})| < 1.3"),"");
    }
  if(j==3)
    {
      legpar->AddEntry("",Form("1.3 #leq |Y(B^{+})| < 1.7"),"");
    }
  if(j==4)
    {
      legpar->AddEntry("",Form("1.7 #leq |Y(B^{+})| #leq 2.4"),"");
    }
...
void BuFit_rapidityCuts(int era = 3, int vq = 1,int m=0)
{
  ...
  for(Long64_t jentry=0; jentry<nentries;jentry++)   
    {
      ...
      // Cortes en el rapidity del B
      //------------------------------

      // |rapidity| < 0.8
      if(j==1)
	{
	  if(abs(t.rapidityB)>=0.8)continue;
	}
      // 0.8 =< |rapidity| < 1.3
      if(j==2)
	{
	  if(abs(t.rapidityB)<0.8 || abs(t.rapidityB)>=1.3)continue;
	}
      // 1.3 =< rapidity < 1.7
      if(j==3)
	{
	  if(abs(t.rapidityB)<1.3 || abs(t.rapidityB)>=1.7)continue;
	}
      // 1.7 =< |rapidity| =< 2.4
      if(j==4)
	{
	  if(abs(t.rapidityB)<1.7 || abs(t.rapidityB)>2.4)continue;
	}
  ...
  ofstream salida_nominal(Form("plotsBines/rapidityPlots/output_BuFit_nominal_%1i_%1i_graph_%1i_Rapidity.txt",era,vq,j));
  ...
  canv_nominal->Print(Form("plotsBines/rapidityPlots/mass_BuFit_era%1i_best%1i_graph_%1i_Rapidity.png",era,vq,j));
  canv_nominal->Print(Form("plotsBines/rapidityPlots/mass_BuFit_era%1i_best%1i_graph_%1i_Rapidity.pdf",era,vq,j));
