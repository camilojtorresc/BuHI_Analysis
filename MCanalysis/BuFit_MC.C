...
void DoFit(RooDataSet data,  RooRealVar M, Double_t maxM, Double_t minM, int vq, TString Lumi);
...
//RooExponential
//RooAddPdf
TCanvas* CreateCanvasNom(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAbsPdf* MassModel, RooAbsPdf* sumgau, RooAbsPdf *bkg1, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, TString Lum, Int_t valsT)
{
...
DoFit(data, M, Mmax, Mmin, vq, Lumi);
...
//------------ Fit procedure -------------------
 ...
 ofstream salida_nominal(Form("plots/output_BuFit_nominal_MC_%1i.txt",vq));
 salida_nominal.is_open();
 save_result(salida_nominal, Ns, Nb);
 
 //made canvas
 TCanvas* canv_nominal = CreateCanvasNom("canv_nominal", fitres, data, M, supM, infM, MassModel, sumgau, bkg1, Ns, Nb, width, width2, fs, mean, Lumi, 0); 
 canv_nominal->Print(Form("plots/mass_BuFit_MC_best%1i.png",vq));
 canv_nominal->Print(Form("plots/mass_BuFit_MC_best%1i.pdf",vq));
}
