...
void DoFit(RooDataSet data,  RooRealVar M, Double_t maxM, Double_t minM, TString Lumi, Double_t ptl, Double_t prob, Double_t cts);
...
TCanvas* CreateCanvasNom(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAbsPdf* MassModel, RooAbsPdf* sumgau, RooAbsPdf *bkg1, RooAbsPdf* genpdf, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, Double_t ptl, Double_t prob, Double_t cts, TString Lum, Int_t valsT)
{
  ...
  //legMass->SetHeader(Form("p_{T}(K^{+}) > %1.1f GeV ",ptl));
  legMass->AddEntry("",Form("p_{T}(K^{+}) > %1.1f GeV ",ptl),"");
...
void BuFit_Sinificancia(Double_t ptk, Double_t prob, Double_t cts)
{
...
//------------ Fit procedure -------------------
 ...
 canv_nominal->Print(Form("plots_signi/mass_BuFit_sig_%1.0f_%1.0f_cts%1.0f.png",PTK,PROB,cts));
 canv_nominal->Print(Form("plots_signi/mass_BuFit_sig_%1.0f_%1.0f_cts%1.0f.pdf",PTK,PROB,cts));
 ...
 ofstream salida_nominal(Form("plots_signi/output_Signif_ptk%1.0f_Bpro%1.0f_cts%1.0f.txt",PTK,PROB, cts));
