...
//RooGaussian
//RooGenericPdf
TCanvas* CreateCanvasNom(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAddPdf MassModel, RooAddPdf sumgau, RooExponential bkg1, RooGaussian genpdf, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, TString Lum, int j)
{
  ...
  TLegend *legpar = new TLegend(0.6,0.48,0.8,0.88);
  ...
  // Legends para cortes del número de Tracks
  
  if(j==1)
    {
      legpar->AddEntry("",Form("0 < Ntrk(B^{+}) #leq 35"),"");
    }
  if(j==2)
    {
      legpar->AddEntry("",Form("35 < Ntrk(B^{+}) #leq 75"),"");
    }
  if(j==3)
    {
      legpar->AddEntry("",Form("75 < Ntrk(B^{+}) #leq 185"),"");
    }
  if(j==4)
    {
      legpar->AddEntry("",Form("185 < Ntrk(B^{+}) #leq 250"),"");
    }
...
void BuFit_Ntracks(int era = 3, int vq = 1,int m=0)
{
  ...
  for(Long64_t jentry=0; jentry<nentries;jentry++)   
    {
      ...
      // Cortes en los tracks del B
      //----------------------------
      
      //Low multiplicity 0 < Ntrk <= 35
      if(j==1)
	{
	  if(t.NtrkvipQ<=0 || t.NtrkvipQ>35 )continue;
	}

      // 35 < Ntrk <= 120
      if(j==2)
	{
	  if(t.NtrkvipQ<=35 || t.NtrkvipQ>75 )continue;
	}

      // 120 < Ntrk <= 185
      if(j==3)
	{
	  if(t.NtrkvipQ<=75 || t.NtrkvipQ>185 )continue;
	}
      
      //High multiplicity 185 <= Ntrk <= 250
      if(j==4)
	{
	  if(t.NtrkvipQ<185 || t.NtrkvipQ>250 )continue;
	}
  ...
  ofstream salida_nominal(Form("plotsBines/ntrkPlots/output_BuFit_nominal_%1i_%1i_graph_%1i_Ntracks.txt",era,vq,j));
  ...
  //made canvas
  TCanvas* canv_nominal = CreateCanvasNom("canv_nominal", fitres, data, M, supM, infM, MassModel, sumgau, bkg1, genpdf, Ns, Nb, width, width2, fs, mean, Lumi,j); 
  canv_nominal->Print(Form("plotsBines/ntrkPlots/mass_BuFit_era%1i_best%1i_graph_%1i_Ntracks.png",era,vq,j));
  canv_nominal->Print(Form("plotsBines/ntrkPlots/mass_BuFit_era%1i_best%1i_graph_%1i_Ntracks.pdf",era,vq,j));
