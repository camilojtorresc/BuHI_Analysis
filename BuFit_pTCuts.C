...
TCanvas* CreateCanvasNom(TString cname, RooFitResult* result, RooDataSet data, RooRealVar M, Double_t supM, Double_t infM,  RooAddPdf MassModel, RooAddPdf sumgau, RooExponential bkg1, RooGaussian genpdf, RooRealVar Ns, RooRealVar Nb, RooRealVar width, RooRealVar width2, RooRealVar fs, RooRealVar mean, TString Lum, int j)
{
  ...
  // Legends para cortes el pt

  if(j==1)
    {
      legpar->AddEntry("",Form("7.0 GeV #leq pT(B^{+}) < 10.0 GeV"),"");
    }
  
  if(j==2)
    {
      legpar->AddEntry("",Form("10.0 GeV #leq pT(B^{+}) < 15.0 GeV"),"");
    }
  if(j==3)
    {
      legpar->AddEntry("",Form("15.0 GeV #leq pT(B^{+}) < 20.0 GeV"),"");
    }
  if(j==4)
    {
      legpar->AddEntry("",Form("20.0 GeV #leq pT(B^{+}) < 50.0 GeV"),"");
    }
  if(j==5)
    {
      legpar->AddEntry("",Form("7.0 GeV #leq pT(B^{+}) < 50.0 GeV"),"");
    }
...
void BuFit_pTCuts(int era = 3, int vq = 1,int m=0)
{
  ...
  for(Long64_t jentry=0; jentry<nentries;jentry++)   
    {
      ...
      //***************************************
       ...
       // Cortes en el pt del B
      //-----------------------

      // 7.0 <= pt <= 10.0
      if(j==1)
	{
	  if(t.Bupt<7.0 || t.Bupt>=11.0)continue;
	}

      // 10.0 < pt <= 15.0
      if(j==2)
	{
	  if(t.Bupt<10.0 || t.Bupt>=15.0)continue;
	}
      // 15.0 < pt <= 20.0
      if(j==3)
	{
	  if(t.Bupt<15.0 || t.Bupt>=20.0)continue;
	}
      // 20.0 < pt <= 50.0
      if(j==4)
	{
	  if(t.Bupt<20.0 || t.Bupt>=50.0)continue;
	}
      if(j==5)
	{
  ...
  ofstream salida_nominal(Form("plotsBines/ptPlots/output_BuFit_nominal_%1i_%1i_graph_%1i_pT.txt",era,vq,j));
  ...
  canv_nominal->Print(Form("plotsBines/ptPlots/mass_BuFit_era%1i_best%1i_graph_%1i_pT.png",era,vq,j));
  canv_nominal->Print(Form("plotsBines/ptPlots/mass_BuFit_era%1i_best%1i_graph_%1i_pT.pdf",era,vq,j));
